...
class NeuralNetwork:
    ...
    def createHiddenLayers(self, hiddenLayers):
        ...
        self.layers.append(Layer(hiddenLayers[0][0], self.numberOfFeatures, hiddenLayers[0][1]))

        # rest of the hidden layers
        for i in range(1, len(hiddenLayers)):
            self.layers.append(Layer(hiddenLayers[i][0], self.layers[-1].numberOfNeurons, hiddenLayers[i][1]))

    def createOutputLayer(self, numberOfClasses, activationFunction):
        self.numberOfClasses = numberOfClasses
        self.layers.append(Layer(self.numberOfClasses, self.layers[-1].numberOfNeurons, activationFunction))
    ...
    def backProp(self):
        ...
        self.deltas[-1] = (self.layers[-1].a - self.classVector.T) * self.layers[-1].getActivationDerivative()
        
        for i in range(numberOfLayers - 2, -1, -1):
            self.deltas[i] = np.matmul(self.layers[i+1].wT.T, self.deltas[i+1]) * self.layers[i].getActivationDerivative()
    ...
    def updateWeightsAndBiases(self):
        ...
        for i in range(numberOfLayers):
            ...
            dw = -self.learningRate * np.matmul(self.deltas[i], previousLayerOutput.T)
            self.layers[i].wT += dw
            
            db = -self.learningRate * np.sum(self.deltas[i], axis = 1, keepdims=True)
            self.layers[i].b += db
    
    def saveNetworkStructureAndParameters(self):
        save_object(self.layers, 'networkStructureAndParameters.txt')
        featuresAndClasses = np.array([self.numberOfFeatures, self.numberOfClasses])
